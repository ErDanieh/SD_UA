...
public class VisitorIniciarSesion extends JFrame {
	...
	public VisitorIniciarSesion(String ipBrooker, String puertoBrooker, String topic, String groupID, String clientID,
			FWQ_Visitor visitante) {
		...
		ActionListener iniciarSesion = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (txtId.getText().isEmpty() || txtPassword.getText().isEmpty()) {
					...
					ErroMsg.setText("");
					producer.sendMsg(txtId.getText() + " " + txtPassword.getText(), horaSolInicio);

					String tokenResult = VisitorIniciarSesion.consumerIniciarSesion(consumer, txtId.getText(),
							horaSolInicio);
					switch (tokenResult) {
					case "ERROR":
						setMensajeError();
					...
					default:
						...
						MapWindow mapWindow = new MapWindow(visitante);
						mapWindow.mostrarVentana(visitante);
						break;
					}
				}
			}
