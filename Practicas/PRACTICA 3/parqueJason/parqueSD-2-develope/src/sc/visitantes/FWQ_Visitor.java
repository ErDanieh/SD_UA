...
public class FWQ_Visitor
{
		...
		for (;;)
		{
			...
			try
			{
				...
				if (this.tokenSesion != null)
				{
					...
					cry.addKey("SDESGUAY");
					String mensaje = null;
					mensaje = cry.encriptar(this.tokenSesion + " " + this.idVisitor + " " + this.color + " " + posAnt + " "
							+ this.posAct);
					System.out.println(mensaje);
					//producer.sendMsg(this.tokenSesion + " " + this.idVisitor + " " + this.color + " " + posAnt + " "
					//		+ this.posAct);
					
					producer.sendMsg(mensaje);
		...
		while (true)
		{
			...
			for (ConsumerRecord record : consumer.getRecords())
			{
				...
				try
				{
					...
					String newMap[][] = stringToMap("" + mensaje);
					this.map = newMap;
					// actualizarDatosAtracciones(record.value().toString());
					actualizarDatosAtracciones(mensaje);
	...
	public void moverPosAct(Producer producer, Position newPos)
	{
		...
		cry.addKey("SDESGUAY");
		String mensaje = null;
		mensaje = cry.encriptar(movimiento);
		System.out.println(mensaje);
	...
	public void salirParque()
	{
		...
		cry.addKey("SDESGUAY");
		String mensaje = null;
		mensaje = cry.encriptar(this.idVisitor + " " + this.tokenSesion);

		// productorSalirParque.sendMsg(this.idVisitor + " " + this.tokenSesion);
		productorSalirParque.sendMsg(mensaje);
	...
	public static void main(String[] args)
	{
		...
		if (args.length != 4)
		{
			System.out.println("Error: argumentos invalidos <IP Registry> <puertoEscucha> <IPBrooker> <PuertoBroker>");
			return;
		}
